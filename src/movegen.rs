...
impl MoveGenerator {
    ...
    pub fn add_move(
        ...
        while bb_to > 0 {
            ...
            // Don't allow king checking himself by his own move
            ...
            // Don't allow discovered checks
            let bb_king = board.king_square(player);
            let mut cloned = board.clone();
            cloned.remove_piece(player, piece, from);
            if self.square_attacked(&cloned, board.side_to_not_move(), bb_king) {
                println!("DISCOVERED CHECK");
                cloned.put_piece(player, piece, from);
                continue;
            }
